...
class ExpiryDateExtractor:
    ...
    EXPIRY_KEYWORDS = ["EXP", "Expiry", "Expiry Date", "Expires", "Expiration", "Expires On", "Date of Expiry", "Until"]
    ISSUE_KEYWORDS = ["Iss", "Issue", "Issued", "Issue Date", "Date of Issue"]

    DATE_PATTERNS = [
        (r"(\d{1,2}[/\-. ]\d{1,2}[/\-. ]\d{4})", "%d/%m/%Y"), # MM/DD/YYYY or DD/MM/YYYY or DD MM YYYY or DD-MM-YYYY
    ...
    def apply_expiry_selection_rules(self, ocr_text, all_dates):
        ...
        """
        # Step 1: Try finding an expiry date using keywords
        expiry_date = self.find_expiry_using_keywords(ocr_text, all_dates)
        if expiry_date:
            return expiry_date
        
        # Step 2: Try selecting a future expiry date
